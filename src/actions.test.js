        ...
        it('creates CONFIRM_ACTIVITY and resolves', () => {
            ...
            const promise = store.dispatch(actions.confirmActivity(registerResolve))
                .then(() => { // return of async actions
                    ...
                    expect(store.getActions()[3].type).toEqual(actions.CLOSE_COMPONENT)
                    expect(store.getActions()[4].type).toEqual(actions.CONFIRM_ACTIVITY)
                    expect(store.getActions()[4].error).toBeFalsy()
                    expect(store.getActions()[5].type).toEqual(actions.DEREGISTER_PENDING)
                })
        ...
        it('creates CONFIRM_ACTIVITY and rejects', () => {
            ...
            const promise = store.dispatch(actions.confirmActivity(registerReject))
                ...
                .then(() => { // return of async actions
                    ...
                    expect(store.getActions()[3].type).toEqual(actions.CLOSE_COMPONENT)
                    expect(store.getActions()[4].type).toEqual(actions.CONFIRM_ACTIVITY)
                    expect(store.getActions()[4].error).toBeTruthy()
                    expect(store.getActions()[5].type).toEqual(actions.DEREGISTER_PENDING)
                })
